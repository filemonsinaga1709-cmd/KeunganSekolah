<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
            ...
            <a href="{{ route(auth()->user()->route_prefix . '.siswa.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 transition w-fit">
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mt-6 flex flex-col sm:flex-row sm:justify-end gap-3">
                <a href="{{ route(auth()->user()->route_prefix . '.siswa.edit', $siswa) }}" 
                   class="bg-yellow-500 hover:bg-yellow-600 text-white font-semibold py-2 px-6 rounded-lg inline-flex items-center justify-center">
                ...
                <form action="{{ route(auth()->user()->route_prefix . '.siswa.destroy', $siswa) }}" 
                    ...
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="w-full sm:w-auto bg-red-500 hover:bg-red-600 text-white font-semibold py-2 px-6 rounded-lg inline-flex items-center justify-center">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
